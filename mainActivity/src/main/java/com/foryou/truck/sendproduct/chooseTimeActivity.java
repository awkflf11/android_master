...
import com.foryou.truck.view.DataAndTimePickDialog.onDataAndTimeSetListener;
...
public class chooseTimeActivity extends BaseActivity {
	...
	private boolean receTimeFlag = false;
	...
	private String TAG = "chooseTimeActivity";

	@BindView(id = R.id.finish_layout, click = true)
	...
	private void InitDataStringArray() {
		...
		int daylimit = Integer.valueOf(limit_day);
	...
	private void InitView() {
		...
		mDataView = (WheelView) findViewById(R.id.data);
		mTime1View = (WheelView) findViewById(R.id.time1);
		mTime2View = (WheelView) findViewById(R.id.time2);
		...
		// setButton(BUTTON_POSITIVE, "确定", new
	...
	private void updateTime1Data() {
		Log.i("dataTime", "updateTime1Data .....");
		mTime1List.clear();
		...
		int currentSecondsCount = date.getHours() * 60 * 60 + date.getMinutes()
				* 60;
		if (currentSecondsCount >= 23 * 60 * 60) {
			currentSecondsCount = 23 * 60 * 60 - 10;
		}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		limit_day = getIntent().getStringExtra("limit_day");
		UtilsLog.i(TAG, "limit_day:" + limit_day);
		if (!TextUtils.isEmpty(limit_day) && TextUtils.isDigitsOnly(limit_day)
				&& !limit_day.equals("0")) {
		} else {
			limit_day = "30";
		}
		receTimeFlag = getIntent().getBooleanExtra("rece_time", false);
	...
	public void onClickListener(int id) {
		...
		case R.id.confirm:
			...
			intent.putExtra("goods_time", mDataString[mCurrentData]);
			intent.putExtra("goods_time_index", "" + mCurrentData);
			if (mTime1String[mCurrent1Time].equals("请选择")) {
				intent.putExtra("goods_time_desc", "");
				intent.putExtra("goods_time_desc_index", "" + 0);
			} else {
				intent.putExtra("goods_time_desc", mTime1String[mCurrent1Time]
						+ "-" + mTime2String[mCurrent2Time]);
				intent.putExtra("goods_time_desc_index", "" + mCurrent1Time);
			}
